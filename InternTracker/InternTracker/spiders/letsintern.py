...
from InternTracker.InternTracker.items import LetsInternItem
import json
import requests
import csv 
...
class LetsIntern(scrapy.Spider):
    ...
    def parse(self, response):
        roles = response.css(".job-title a h4::text").getall()
        roles=[i.rstrip() for i in roles]
        roles=[i for i in roles if i !=""]

        companies = response.css(".company-name .truncate-normal a::text").getall()
        companies = [x.replace('\n','').strip() for x in companies]

        locations = response.css(".job-locations .truncate-normal a::text").getall()

        intern_pay_start = response.css(".list-group-item a::text").getall()
        intern_pay_start = [i for i in intern_pay_start if i.replace('\n','').strip() not in ["Starts","Featured",""," "]]
        internship,pay,start_date = intern_pay_start[0::3],intern_pay_start[1::3],intern_pay_start[2::3]
        ...
        contents=c['featured_jobs']

        #open a file for writing
        data_file = open('letsintern_by_json.csv', 'w')
        ...
        for i in range(len(roles)):
            ...
            posting['start_date'] = start_date[i]
            posting['stipend'] = pay[i]
            yield posting
